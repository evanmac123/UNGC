...
class ContributorsQueryTest < ActiveSupport::TestCase
  ...
  should "include only contributions from organizations that match the query" do
    campaign = create_campaign(name: 'LEAD 2014 happy')
    participant = create_organization(name: 'my org', participant: true)
    ...
    participant2 = create_organization(name: 'my not org', participant: true)
    c2 = create_contribution(
      campaign: campaign,
  ...
  should "should ignore accents" do
    campaign = create_campaign(name: 'LEAD 2014 happy')
    participant = create_organization(name: 'Fundación Bancaria "la Caixa"', participant: true)
    c1 = create_contribution(
      campaign: campaign,
      organization: participant,
      raw_amount: 66_000.00,
