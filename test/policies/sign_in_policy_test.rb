...
class SignInPolicyTest < ActiveSupport::TestCase
  ...
  should "allow Network report recipients to sign-in as Contact points from the same local network" do
  ...
  should "NOT allow Network report recipients to sign-in as Contact points from another local network" do
    # Given a network report recipient
    network = create_local_network
    nrr = create_contact(local_network: network, roles: [Role.network_report_recipient])
  ...
  should "NOT allow Network report recipients to sign-in as a non-Contact points" do
    # Given a Network report recipient
    network = create_local_network
    nrr = create_contact(local_network: network, roles: [Role.network_report_recipient])
  ...
  should "allow Network report recipients to sign-in as Contact points in general" do
    ...
    policy = SignInPolicy.new(nrr)
    assert policy.can_sign_in_as_contact_points?, "The Networt report recipient should be able to sign as Contact points"
  end
  ...
  context "scoping down the organizations" do
    ...
    should "exclude unapproved organizations" do
