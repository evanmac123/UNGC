class InitiativesContactsBase < SimpleReport
  ...
  def records
    ...
    WHERE init.name = #{ActiveRecord::Base.sanitize(options[:initiative_name])}
      AND r.id = #{options[:role_id]}
      AND o.cop_state != 'delisted'
    ORDER BY o.id, c.id")
  end
