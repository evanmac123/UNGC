module Admin::OrganizationsHelper
  def organization_actions(organization)
    ...
    unless current_contact.from_organization?
      ...
      actions << button_to('Resume Review', admin_organization_comments_path(@organization.id, :commit => ApprovalWorkflow::EVENT_RESUME_REVIEW), :method => :post, :data => { :confirm => "This application will be put back into review"}, :class => "nobutton") if organization.can_resume_review?
