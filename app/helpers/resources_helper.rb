module ResourcesHelper
  ...
  def languages_for_select(selected_language=nil)
    languages = ResourceLink.includes(:language).joins(:resource).where("resources.approval='approved'").group('language_id').select(:language_id).map { |c| [c.language.name, c.language_id] }
